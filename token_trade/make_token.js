...
// 토큰을 거래하는 함수 
async function trans_token(_token_address, _address, _amount){
    // 발행한 토큰을 keyringContainer에 지정
    const kip7 = await new caver.kct.kip7(_token_address)
...
async function trans_from_token(_token_address,_address, _amount){
    // 발행한 토큰을 keyringContainer에 지정
    const kip7 = await new caver.kct.kip7(_token_address)
    kip7.setWallet(keyringContainer)
    ...
    const keyring2 = keyringContainer.keyring.createFromPrivateKey(_address) 
    ...
    const receipt = await kip7.transferFrom(keyring2.address, owner, _amount, {from : keyring.address})
